...
from mopidy_tunein import parsers

type Detector = Callable[[bytes], bool]
...
def test_detect_from_valid_header(detect_fn: Detector, data: bytes) -> None:
...
def test_detect_from_invalid_header(detect_fn: Detector) -> None:
...
def test_parse_given_format_from_valid_data(parse_fn: Parser, data: bytes) -> None:
...
def test_parse_given_format_from_invalid_data(parse_fn: Parser) -> None:
...
def test_parse_any_format_from_valid_data(data: bytes) -> None:
    assert parsers.parse_playlist(data) == EXPECTED


def test_parse_from_invalid_data() -> None:
...
def test_parse_malformed_pls(data: bytes, expected: list[str]) -> None:
    assert list(parsers.parse_pls(data)) == expected


def test_parse_malformed_asx_reference() -> None:
